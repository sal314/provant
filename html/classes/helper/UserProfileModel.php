...
*/
    ...
    public function updateUser(){
    	$sql="UPDATE z_users SET 
    	       `first_name` = '".$this->dbOb->escape_string($this->data['first_name'])."',
    	       `last_name`  = '".$this->dbOb->escape_string($this->data['last_name'])."',
    	       `email` = '".$this->dbOb->escape_string($this->data['email'])."',
    	       	`password` = PASSWORD('".$this->dbOb->escape_string($this->data['password'])."')
    	       	WHERE id = ".$this->id;
    	$this->dbOb->update($sql);     	  		
    
    	//$this->id=$this->dbOb->update($sql);
    ...
    //	print_r($sql);
    //	exit;
    	$statussql = "UPDATE u_profile SET 
    				  `status` = 'active'
    				  WHERE `z_user_id` = '".$this->dbOb->escape_string($this->id)."'";
    	//print_r($statussql);
    	//exit;
    	$this->dbOb->query($statussql);
