 ...
 */
 ...
 class pChart
  {
   /* Palettes definition */
   private  $Palette = array("0"=>array("R"=>188,"G"=>224,"B"=>46),
   ...
   private $ErrorInterface = "CLI";
   private $Errors         = NULL;
   private $ErrorFontName  = "Fonts/pf_arma_five.ttf";
   private $ErrorFontSize  = 6;

   /* privates related to the graphing area */
   ...
   private $XDivisions      = NULL;
   private $DivisionHeight  = NULL;
   private $XDivisionHeight = NULL;
   private $DivisionCount   = NULL;
   private $XDivisionCount  = NULL;
   private $DivisionRatio   = NULL;
   private $XDivisionRatio  = NULL;
   private $DivisionWidth   = NULL;
   private $DataCount       = NULL;
   private $Currency        = "\$";

   /* Text format related privates */
   private $FontName       = NULL;
   private $FontSize       = NULL;
   private $DateFormat     = "d/m/Y";

   /* Lines format related privates */
   private $LineWidth      = 1;
   private $LineDotSize    = 0;

   /* Layer related privates */
   private $Layers         = NULL;

   /* Set antialias quality : 0 is maximum, 100 minimum*/
   private $AntialiasQuality = 0;

   /* Shadow settings */
   private $ShadowActive    = FALSE;
   private $ShadowXDistance = 1;
   private $ShadowYDistance = 1;
   private $ShadowRColor    = 60;
   private $ShadowGColor    = 60;
   private $ShadowBColor    = 60;
   private $ShadowAlpha     = 50;
   private $ShadowBlur      = 0;

   /* Image Map settings */
   private $BuildMap         = FALSE;
...
// * Fixed in 1.27 *         { $X2 = $this->GArea_X2 - 1; $Y2 = $this->GArea_Y2 - ($this->GArea_X2 - $X1); }
  ...
  function drawScale($Data,$DataDescription,$ScaleMode,$R,$G,$B,$DrawTicks=TRUE,$Angle=0,$Decimals=1,$WithMargin=FALSE,$SkipLabels=1,$RightScale=FALSE)
       ...
       if ( $ID % $SkipLabels == 0 )
        {
         ...
         $TextWidth  = $Position[2]+abs($Position[0]);                  
         $TextHeight = $Position[1]+abs($Position[3]);
         /*
          * interesting the upper and lowe coords do not create a square but a paralellagram or rhombus 
          * needed to get the max corners to get teh width
          * */ 
		 ...
		 { 
 		   //imagettftext does a piss poor job of roating text so I swapped out the code to use the imagerotate (which is less bad)
 		   $Position   = imageftbbox($this->FontSize,0,$this->FontName,$Value);
           $LTextWidth  = max(abs($Position[2]),abs($Position[4]))+max(abs($Position[0]),abs($Position[6]));         
           $LTextHeight = max(abs($Position[1]),abs($Position[7]))+max(abs($Position[3]),abs($Position[5]));
           $LTextMinHeight = min(abs($Position[1]),abs($Position[7]))+min(abs($Position[3]),abs($Position[5]));
         // 0 ll x  1 ll y
         // 2 lr x  3 lr y
         // 4 ur x  5 ur y
         // 6 ul x  7 ul y 
           ...
           $XPosText=$XPos-(.5*$LTextHeight);
              
           $im = imagecreatetruecolor($LTextWidth, $LTextHeight);
           imagesavealpha($im, true);
           $trans_colour = imagecolorallocatealpha($im, 0, 0, 0, 127);
           imagefill($im, 0, 0, $trans_colour);
           imagettftext($im,$this->FontSize,0,0,$LTextHeight-$LTextMinHeight,$C_TextColor,$this->FontName,$Value);           
           //$im2=imagerotate($im,$Angle,$trans_colour,$trans_colour);
           $im2=imageRotate2($im,$Angle,false);
           imagedestroy($im);       
               
           imagecopy($this->Picture,$im2,$XPosText,$YPos,0,0,imagesx($im2)  ,imagesy($im2));           
           imagedestroy($im2);
           $YPos = $this->GArea_Y2+10+$LTextWidth;
         }
         /*
          {
    ...
    if ( isset($DataDescription["Axis"]["X"]) )
      {
       ...
       //changed to center on the graph area                 
       $TextLeft   = (($this->GArea_X2-$this->GArea_X1) / 2) - ($TextWidth/2)+($this->GArea_X1);
   ...
   function drawBarGraph($Data,$DataDescription,$Shadow=FALSE,$Alpha=100)
    ...
    function drawBar3DGraph($Data,$DataDescription,$Shadow=FALSE,$Alpha=100)
    {
     /* Validate the Data and DataDescription array */
     $this->validateDataDescription("drawBarGraph",$DataDescription);
     $this->validateData("drawBarGraph",$Data);
     ...
     $SeriesWidth  = $this->DivisionWidth / ($Series+1);
     $SerieXOffset = $this->DivisionWidth / 2 - $SeriesWidth / 2;
     $barWidth=floor($SeriesWidth*.4);
     $barHeight=floor($barWidth/2);
           ...
           if ( is_numeric($Data[$Key][$ColName]) )
            {
             ...
             if($Value>=0){
             	if ( $Shadow && $Alpha == 100 ){
              		$this->drawRectangle($XPos+1,$YZero,$XPos+$SeriesWidth-1,$YPos,25,25,25,TRUE,$Alpha);
              		$this->drawRectangle($XPos+1,$YZero,$XPos+$SeriesWidth-$barHeight,$YPos,25,25,25,TRUE,$Alpha);
             	}
             	if($YPos-5>=$YZero) $off=-$barHeight;
             	else if($YPos<$YZero) $off=$barHeight;
             	else $off=$YZero-$YPos;
             	$y2=$YPos+$off;
             	//bg alphaed
             	$this->drawFilledRectangle($XPos+$barWidth,$YZero,$XPos+$SeriesWidth,$YPos,$this->Palette[$ColorID]["R"],$this->Palette[$ColorID]["G"],$this->Palette[$ColorID]["B"],TRUE,$Alpha);
             	//fg solid             
             	$this->drawFilledRectangle($XPos,$YZero,$XPos+$SeriesWidth-$barWidth,$y2,$this->Palette[$ColorID]["R"],$this->Palette[$ColorID]["G"],$this->Palette[$ColorID]["B"],TRUE,100);

             	//triangle between fg and bg
             	$points=array($XPos+1,$y2,
             			   $XPos+$barWidth+1,$YPos,
             			   $XPos+$barWidth+1,$y2
             	);             
             	$this->drawFilledPolygon($points,$this->Palette[$ColorID]["R"],$this->Palette[$ColorID]["G"],$this->Palette[$ColorID]["B"],false,$Alpha);
             
             	//connection lines between fg and bg corners
             	$this->drawLine($XPos+$SeriesWidth-$barWidth,$y2,$XPos+$SeriesWidth,$YPos,$this->Palette[$ColorID]["R"],$this->Palette[$ColorID]["G"],$this->Palette[$ColorID]["B"]);
             	$this->drawLine($XPos+1,$y2,$XPos+$barWidth,$YPos,$this->Palette[$ColorID]["R"],$this->Palette[$ColorID]["G"],$this->Palette[$ColorID]["B"]);
             }else{
				if ( $Shadow && $Alpha == 100 ){
              		$this->drawRectangle($XPos+1,$YZero,$XPos+$SeriesWidth-1,$YPos,25,25,25,TRUE,$Alpha);
              		$this->drawRectangle($XPos+1,$YZero,$XPos+$SeriesWidth-$barHeight,$YPos,25,25,25,TRUE,$Alpha);
             	...
             	$y2=$YPos+$off;
             	//bg alphaed
             	$this->drawFilledRectangle($XPos,$YZero,$XPos+$SeriesWidth-$barWidth,$y2,$this->Palette[$ColorID]["R"],$this->Palette[$ColorID]["G"],$this->Palette[$ColorID]["B"],TRUE,$Alpha);             	
             	//fg solid             
             	$this->drawFilledRectangle($XPos+$barWidth,$YZero,$XPos+$SeriesWidth,$YPos,$this->Palette[$ColorID]["R"],$this->Palette[$ColorID]["G"],$this->Palette[$ColorID]["B"],TRUE,100);
             	
             	//triangle between fg and bg
             	$points=array($XPos,$y2,
             			   $XPos+$barWidth+1,$y2,
             			   $XPos+$barWidth+1,$YPos
             	);             
             	...
             	//connection lines between fg and bg corners
             	$this->drawLine($XPos,$y2,$XPos+$barWidth,$YPos,$this->Palette[$ColorID]["R"],$this->Palette[$ColorID]["G"],$this->Palette[$ColorID]["B"]);
  ...
  function drawFilledPolygon($coords,$R,$G,$B,$DrawBorder=TRUE,$Alpha=100,$NoFallBack=FALSE)
    {
    	...
    	$normalX=min($Xc);
    	$normalY=min($Yc);
    	$LayerWidth=max($Xc)-$normalX;
    	$LayerHeight=max($Yc)-$normalY+1;
       ...
       $uCoords=array();
		for($x=0;$x<sizeof($coords);$x++){
			$uCoords[$x]=$coords[$x]-$normalX;
			$x++;
			$uCoords[$x]=$coords[$x]-$normalY;
		}
	    
       imagefilledpolygon($this->Layers[0],$uCoords,sizeof($uCoords)/2,$C_Rectangle);
       imagecopymerge($this->Picture,$this->Layers[0],round($normalX-1),round($normalY-1),0,0,$LayerWidth,$LayerHeight,$Alpha);
       imagedestroy($this->Layers[0]);
      
      if ( $DrawBorder )
      {
       $ShadowSettings = $this->ShadowActive; $this->ShadowActive = FALSE;
       $this->drawPolygon($coords,$R,$G,$B);
  ...
  /* This function create a rectangle with antialias */
   function drawPolygon($coords,$R,$G,$B)
    {
     if ( $R < 0 ) { $R = 0; } if ( $R > 255 ) { $R = 255; }
     if ( $G < 0 ) { $G = 0; } if ( $G > 255 ) { $G = 255; }
     if ( $B < 0 ) { $B = 0; } if ( $B > 255 ) { $B = 255; }
   ...
   function isRealInt($Value)
    ...
    function setAntialiasQuality($v){
    	$this->AntialiasQuality=$v;
    }
  	function getPicture(){
    	return $this->Picture;
    }
