...
class LabVoucherKitModel {
	...
	public function __construct($id=0) {
		...
		$sql = "SELECT * FROM u_lab_voucher_kit_order WHERE z_user_id='" . $this->dbOb->escape_string ( $this->id ) . "'";
		...
		if ($rec) { //we order a kit are we waiting for it or have we received it?
			...
			$this->orderStatus = ($rec ['is_received']==0) ? 2 : 3;			
			return;
		}
	...
	public function getWaitStatus() {
		if($this->orderStatus==2){
			$sql="SELECT date_added>date_sub(CURRENT_DATE(), INTERVAL 6 DAY) FROM u_lab_voucher_kit_order WHERE z_user_id='".$this->dbOb->escape_string($this->id)."'";
	...
	public function sendOrder() {
		...
		$v=implode(",",$values);
		//add to database
		$sql = "INSERT INTO u_lab_voucher_kit_order(z_user_id,".$k.") VALUES (
		'".$this->dbOb->escape_string($this->id)."',".$v.")";		
		$this->dbOb->insert ( $sql );
	}
	
	public function getScreeningResults(){
		$sql="SELECT * FROM u_lab_voucher_kit_results WHERE z_user_id='".$this->dbOb->escape_string($this->id)."'";
		...
		$bgm=$bgm->getLastEntry();
		$results['blood_glucose']=$bgm['blood_glucose'];
