...
class ModuleMoveIT  extends PageBase{
	private $mod = null;
	...
	public function __construct() {
		...
		if ($this->cred->hasRead("LOGIN_HEALTH_COACH")) {
			$this->mod = new ModuleMoveITModel(true);
		}
		else {
			$id = $this->cred->getID();
			...
			$sql = "SELECT * FROM p_company_modules WHERE p_company_id='".$this->dbOb->escape_string($cid)."' AND p_module_id='6'";
  			if(!$this->dbOb->getOne($sql))throw new Exception("This module is not available");

			$this->mod = new ModuleMoveITModel();
		}
	}
	...
	public function Index($param){	
		...
		$template=TemplateParser::enqueue(TEMPLATE_DIR."frontend/ITModules/MoveIt/".$t);
	...
	public function Intro($param){
		...
		if(count($_POST) && $page==3){
			...
			if (!$result['err']) {
				$this->mod->update();
				header("Location: /ModuleMoveIT/Index/");
				exit();
			}
		}
		if(!count($_POST) && $page==3){
		...
		$template=TemplateParser::enqueue(TEMPLATE_DIR."frontend/ITModules/MoveIt/intro_".$page.".tpt");
	...
	public function Week($param){
		...
		$template = TemplateParser::enqueue(TEMPLATE_DIR."frontend/ITModules/MoveIt/week".$week."_".$page.".tpt");	
		
		$pages = 1;
		$numPages = array(0, 7, 10, 6, 8, 7);
		if (count($_POST)) {
			$ftt = 1;
			$validateData = false;
			switch($week) {
			...
			if ($validateData) {
				...
				if (!$result['err']) {
					$this->mod->update();
					if ($attempt == 1) {  //If just an exersice page
						header("Location: /ModuleMoveIT/Week/".$week."/".(++$page));
						exit();
					}
					$template->addVar("submit", 1);		//All answers correct - allow next page
				}
				else {
					if ($attempt > 3) {
						$template->addVar("submit", 2);	//Third try - take 'em back to the start of the module
					}
					else {
						$template->addVar("submit", 3);	//Allow them to correct their answers
					}
				}
			}
			else {
	...
	public function info($param){
		$page = $param[0];
		$template = TemplateParser::create(TEMPLATE_DIR."frontend/ITModules/MoveIt/".$page."_pop.tpt");
		$template->parse();
		exit();
	}
	...
	public function Download($param) {
		...
		$pdf->SetTitle('MoveIT Completion');
		$pdf->SetSubject('Completion Certificate');
		$pdf->SetKeywords('Provant MoveIT Completion Certificate');
		...
		// Image example
//		error_log(ROOT_DIR.'uploads/file_upload/ITModules/MoveIT/cert.jpg');
		$pdf->Image(ROOT_DIR.'assets/media/images/myfocus/moveit/certificate.jpg', 0, 0, 280,0,
		...
		$pdf->Output('MoveIT_Completion.pdf', 'I');
	}
	...
	public function Start ($params) {
		...
		$template = TemplateParser::enqueue(TEMPLATE_DIR."frontend/healthcoach/moveit.tpt");
		...
		if (count($user) == 0) {
			...
			$sql = "SELECT * FROM u_module_moveit " .
			...
			if (count($data) == 0) {
				...
				$nodata->name = "";
				$nodata->type = 5;
				$nodata->message = $user['first_name'] . " " . $user['last_name'] . " has not taken the Move IT module";
				$err[] = $nodata;				
			}
			else {
