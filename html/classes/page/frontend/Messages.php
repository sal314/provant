...
*/
  ...
  class Messages extends PageBase{
  		...
  		public function Inbox($params){  			
  			...
  			switch($s){
  				case 2: 
  					...
  					$page=$params[1];
  					$ordpath=$params[0];
					$archive_url =  "/".$this->getName()."/ArchiveAllMesssage/".$params[0]."/".$params[1];
					$delete_url =  "/".$this->getName()."/DeleteInboxMesssage/".$params[0]."/".$params[1];
  					break;
  				case 3:
  					$orderBy=$params[0];
  					$dir=$params[1];
					...
					$archive_url =  "/".$this->getName()."/ArchiveAllMesssage/".$params[0]."/".$params[1]."/".$params[2];
					$delete_url =  "/".$this->getName()."/DeleteInboxMesssage/".$params[0]."/".$params[1]."/".$params[2];
  					break;
  				default: 					
  					$ordpath=$orderBy;
					$archive_url =  "/".$this->getName()."/ArchiveAllMesssage/".$ordpath;
					$delete_url =  "/".$this->getName()."/DeleteInboxMesssage/".$ordpath;
  					break;  					
  			}
			...
			$template->addVar("archive_url", $archive_url);
			$template->addVar("delete_url", $delete_url);
  		}
		
		
		public function ArchiveAllMesssage($params){
			$mode=MessageModel::ALL;
  			$page=1;
  			$orderBy=MessageModel::DATESENT;
  			$dir=MessageModel::DESC;
  			...
  			switch($s){
  				case 2: 
  					...
  					$page=$params[1];
  					$ordpath=$params[0];
					$redirect_url =  "/".$this->getName()."/Index/".$params[0]."/".$params[1];
  					break;
  				case 3:
  					$orderBy=$params[0];
  					$dir=$params[1];
  					$page=$params[2];
  					$ordpath=$params[0]."/".$params[1];		
					$redirect_url =  "/".$this->getName()."/Index/".$params[0]."/".$params[1]."/".$params[2];
  					break;
  				default: 					
  					$ordpath=$orderBy;	
					$redirect_url =  "/".$this->getName()."/Index/".$ordpath;
  					break;  					
  			}
			
			$messages = $this->message->getMessageHeaders(MessageModel::ALL,$page,$orderBy,$dir);
			$msgcount = count($messages);
			if($msgcount > 0){
				foreach($messages as $msgkey=>$msgval){
					$mid = isset($_REQUEST["msg_".$msgkey])?$_REQUEST["msg_".$msgkey]:'';
					$mid = intval($mid);					
					if($mid == $msgval["id"]){
						$ret = $this->message->archiveMessage($mid, $this->cred->getId());	
					}							
				}
			}	
		 ...
		 public function DeleteInboxMesssage($params){
			$mode=MessageModel::ALL;
  			$page=1;
  			$orderBy=MessageModel::DATESENT;
  			$dir=MessageModel::DESC;
  		...
  		public function Archive($params){
  			...
  			switch($s){
  				case 2: 
  					...
  					$page=$params[1];
  					$ordpath=$params[0];
					$delete_url =  "/".$this->getName()."/DeleteArchiveMesssage/".$params[0]."/".$params[1];
  					break;
  				case 3:
  					$orderBy=$params[0];
  					$dir=$params[1];
  					$page=$params[2];
  					$ordpath=$params[0]."/".$params[1];
					$delete_url =  "/".$this->getName()."/DeleteArchiveMesssage/".$params[0]."/".$params[1]."/".$params[2];
  					break;
  				default: 					
  					$ordpath=$orderBy;
					$delete_url =  "/".$this->getName()."/DeleteArchiveMesssage/".$ordpath;
  					break;  					
  			}
		...
		public function DeleteArchiveMesssage($params){
			$mode=MessageModel::ALL;
  			$page=1;
  			$orderBy=MessageModel::DATESENT;
  			$dir=MessageModel::DESC;
  			...
  			switch($s){
  				case 2: 
  					...
  					$page=$params[1];
  					$ordpath=$params[0];
					$redirect_url =  "/".$this->getName()."/Archive/".$params[0]."/".$params[1];
  					break;
  				case 3:
  					$orderBy=$params[0];
  					$dir=$params[1];
  					$page=$params[2];
  					$ordpath=$params[0]."/".$params[1];	
					$redirect_url =  "/".$this->getName()."/Archive/".$params[0]."/".$params[1]."/".$params[2];
  					break;
  				default: 					
  					$ordpath=$orderBy;
					$redirect_url =  "/".$this->getName()."/Archive/".$ordpath;
  					break;  					
  			}
			
			$messages = $this->message->getArchivedMessages(MessageModel::ALL,$page,$orderBy,$dir);
			$msgcount = count($messages);
			if($msgcount > 0){
				foreach($messages as $msgkey=>$msgval){
					$mid = isset($_REQUEST["msg_".$msgkey])?$_REQUEST["msg_".$msgkey]:'';
  		...
  		public function Sent($params){  			
  			...
  			switch($s){
  				case 2: 
  					...
  					$page=$params[1];
  					$ordpath=$params[0];
					$delete_url =  "/".$this->getName()."/DeleteSentMesssage/".$params[0]."/".$params[1];
  					break;
  				case 3:
  					$orderBy=$params[0];
  					$dir=$params[1];
  					$page=$params[2];
  					$ordpath=$params[0]."/".$params[1];
					$delete_url =  "/".$this->getName()."/DeleteSentMesssage/".$params[0]."/".$params[1]."/".$params[2];
  					break;
  				default: 					
  					$ordpath=$orderBy;
					$delete_url =  "/".$this->getName()."/DeleteSentMesssage/".$ordpath;
  					break;  					
  			}
		...
		public function DeleteSentMesssage($params){
			$mode=MessageModel::ALL;
  			$page=1;
  			$orderBy=MessageModel::DATESENT;
  			$dir=MessageModel::DESC;
  			...
  			switch($s){
  				case 2: 
  					...
  					$page=$params[1];
  					$ordpath=$params[0];
					$redirect_url =  "/".$this->getName()."/Sent/".$params[0]."/".$params[1];
  					break;
  				case 3:
  					$orderBy=$params[0];
  					$dir=$params[1];
  					$page=$params[2];
  					$ordpath=$params[0]."/".$params[1];
					$redirect_url =  "/".$this->getName()."/Sent/".$params[0]."/".$params[1]."/".$params[2];
  					break;
  				default: 					
  					$ordpath=$orderBy;
					$redirect_url =  "/".$this->getName()."/Sent/".$ordpath;
  					break;  					
  			}
			
			$messages = $this->message->getMessageHeaders(MessageModel::SENT,$page,$orderBy,$dir);
			$msgcount = count($messages);
			if($msgcount > 0){
				foreach($messages as $msgkey=>$msgval){
					$mid = isset($_REQUEST["msg_".$msgkey])?$_REQUEST["msg_".$msgkey]:'';
