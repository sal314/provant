...
//return $template;
			...
			$sql="SELECT pia.description, pitl.points, pitl.date_added FROM p_incentive_triggers_log AS pitl".
					" JOIN p_incentive_triggers AS pit ON pitl.p_incentive_triggers_id=pit.id".
					" JOIN p_incentive_activity AS pia ON pit.incentive_activity_id=pia.id".
					" WHERE pitl.z_user_id=".$this->cred->getId() .
					" ORDER BY pitl.date_added DESC";
					//" AND pia.is_active=1".
					//" AND pia.module <> 'Self-reporting'";
			
			$dha_points=$this->dbOb->query($sql);
			if (!$dha_points) {
				throw new Exception("No Healthy Achievements points");
			}
			else {
				foreach ($dha_points as &$pts) {
					$pts['date_added'] = date('M j, Y h:i:s', strtotime($pts['date_added']));
				}
			}
			...
			$template->addVar("points", $dha_points);
